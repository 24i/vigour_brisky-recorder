...
// lets record stamps and use them
var recordings = {}
...
module.exports = {
  ...
  play: function (speedup = 1, i = last, state) {
    paused = true
    if (!state) { state = global.state }
    ...
    function doit () {
      setTimeout(() => {
        ...
        console.log(stamp, recordings[records[i]][0] - offset)
        for (var j = 1; j < recordings[records[i]].length; j++) {
          state.set(recordings[records[i]][j], stamp)
        }
        bs.close(stamp)
      ...
      },  speedup * (recordings[records[i]][0] - offset))
    }
    doit()
  }
}
